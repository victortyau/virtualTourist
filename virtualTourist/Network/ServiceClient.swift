...
class ServiceClient {
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            switch self {
            case .searchPhotos(let lat, let long, let page): return Endpoints.baseUrl + "?method=flickr.photos.search&format=json&api_key=\(Endpoints.apiKey)&radius=10&lat=\(lat)&lon=\(long)&nojsoncallback=1&page=\(page)&per_page=\(Int.random(in: 1...10))"
            }
        }
